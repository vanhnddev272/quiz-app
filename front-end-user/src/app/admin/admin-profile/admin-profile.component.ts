...
export class AdminProfileComponent implements OnInit {
  ...
  getUserInfo() {
    this.userService.getUserInfo("").subscribe((res) => {
      ...
      this.imgUpload =
        this.userProfile.profile.image ||
        "https://isc-quiz.s3-ap-southeast-1.amazonaws.com/default.png";
    });
  }
  ...
  updatePassword(data) {
    this.userService.updatePassword(this.userProfile.id, data).subscribe(
      (res) => {
  ...
  getAvatarUpload(urlUpload) {
    ...
    this.uploadService.uploadAvatar(this.imgFile).subscribe(
      (res) => {
        this.imgUpload = res;
      ...
      (error) => {
        this.toast.error(
          "Some problems. Report it to the administrator",
          "Error"
        );
      }
    );
